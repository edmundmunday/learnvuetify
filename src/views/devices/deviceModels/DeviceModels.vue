...
export default {
  ...
  methods: {
    ...
    //this gets called when a new model is added
    deviceModelAdded() {
      this.text = "Device Model Successfully Added!";
      this.snackbar = true;
    },
    refreshParent(doc, index) {
      console.log("refresh was called");

      this.deviceModels[index].manufacturer = doc.title;
      this.componentKey++;

      // this.deviceModels.forEach(model => {
  ...
  created() {
    db.collection("deviceModels").onSnapshot(response => {
      const deviceModelsArray = [];

      response.forEach(response => {
        deviceModelsArray.push({ ...response.data(), id: response.id });
      });

      deviceModelsArray.map((model, index) => {
        db.collection("deviceManufacturers")
          .doc(model.parent)
          .get()
          .then(snapshot => {
            this.refreshParent(snapshot.data(), index);

            // this.deviceModels[index].manufacturer = snapshot.data().title;
            // this.componentKey++;
          });
      });
      this.deviceModels = deviceModelsArray;
      // this.loading = false;
    });
  }
};
